...
global _instances: table[string] of ClusterController::Types::Instance = table();
...
global _instances_ready: set[string] = set() &on_change=check_instances_ready;
...
# This is the &on_change handler for the _instances_ready set.
function check_instances_ready(insts: set[string], tc: TableChange, inst: string)
	...
	if ( tc == TABLE_ELEMENT_NEW || tc == TABLE_ELEMENT_REMOVED )
		...
		if ( cur_instances == _instances_ready )
			{
...
function drop_instance(inst: ClusterController::Types::Instance)
	...
	delete _instances[inst$name];

	if ( inst$name in _instances_ready )
		delete _instances_ready[inst$name];
...
event ClusterController::API::notify_agents_ready(instances: set[string])
	...
	local req = ClusterController::Request::lookup(_config_reqid_pending);
...
event ClusterAgent::API::notify_agent_hello(instance: string, host: addr, api_version: count)
	...
	local inst = _instances[instance];
...
event ClusterAgent::API::set_configuration_response(reqid: string, result: ClusterController::Types::Result)
	...
	# This is the point where we're really done with the original
        ...
        # one.
	_config_current = req$set_configuration_state$config;
...
event ClusterController::API::set_configuration_request(reqid: string, config: ClusterController::Types::Configuration)
	...
	if ( _config_reqid_pending != "" )
		{
		res = ClusterController::Types::Result($reqid=reqid);
		res$success = F;
		res$error = fmt("request %s still pending", _config_reqid_pending);
		req$results += res;
	...
	for ( inst in config$instances )
		...
		# Broker ID?
		if ( ( inst$host != 0.0.0.0 && inst$host != _instances[inst$name]$host ) ||
		     inst?$listen_port != _instances[inst$name]?$listen_port ||
		     ( inst?$listen_port && _instances[inst$name]?$listen_port &&
		       inst$listen_port != _instances[inst$name]$listen_port ) )
			{
	...
	for ( inst_name in insts_to_drop )
		drop_instance(_instances[inst_name]);
