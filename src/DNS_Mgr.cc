...
class DNS_Request
	{
public:
	DNS_Request(const char* host, int af, int request_type, bool async = false);
...
DNS_Request::DNS_Request(const char* host, int af, int request_type, bool async)
	: host(util::copy_string(host)), family(af), request_type(request_type), async(async)
	{
	}
...
void DNS_Request::MakeRequest(ares_channel channel)
	...
	if ( request_type == T_A || request_type == T_AAAA )
		...
		int len = 0;
		int status = ares_create_query(host, C_IN, request_type, DNS_Request::request_id, 1, &query,
		                               &len, 0);
